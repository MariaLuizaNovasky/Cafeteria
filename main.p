  ...
  repeat
    ...
    begin
     ...
     if respc = 0 then
      ...
      quantidade[i] := qtd;      //quantidade de cada item pedido
      totalItens := totalItens + qtd;      // total de itens pedidos
      total := (total + comidasPrecos[respc]) * qtd;  // Cálculo do valor do pedido
      
      Writeln('');      // Mensagem de adição do pedido
    ...
    if respm = 2 then
    begin 
      Writeln('Cardápio de bebidas');  // Cardápio de Bebidas
     ...
     if respb = 0 then
      ...
      quantidade[i] := qtd;   // Quantidade de cada item pedido      
      totalItens := totalItens + qtd;   // Total de itens pedidos
      total := (total + bebidasPrecos[respb] ) * qtd;  // Cálculo do valor total do pedido
      
      Writeln('');    // Mensagem de confirmação
      Writeln('Pedido adicionado! Preço atual do pedido: R$', total:0:2);
      Writeln('----------------------------------------');
      i := i + 1;
    end;
